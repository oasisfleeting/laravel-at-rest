...
class ConfigController extends Controller {
	...
	{	
		$this->data['active'] = '';
		return view('spnet.config.index',$this->data);
	}
		...
		if (!$validator->fails()) 
		{
			...
			if(!is_null(Input::file('logo')))
			{
				...
				$file = Input::file('logo'); 
			 	$destinationPath = public_path().'/spnet/images/';
			...
			fwrite($fp,$val); 
			fclose($fp); 
			return Redirect::to('spnet/config')->with('messagetext','Setting Has Been Save Successful')->with('msgstatus','success');
		} else {
			return Redirect::to('spnet/config')->with('messagetext', 'The following errors occurred')->with('msgstatus','success')
	...
	public function getEmail()
	{
		...
		return view('spnet.config.email',$this->data);
	...
	function postEmail( Request $request)
	{
		...
		if ($validator->passes()) 
		{
			...
			return Redirect::to('spnet/config/email')->with('messagetext', 'Email Has Been Updated')->with('msgstatus','success');
			
		}	else {

			return Redirect::to('spnet/config/email')->with('messagetext', 'The following errors occurred')->with('msgstatus','success')
			->withErrors($validator)->withInput();
		}
	...
	public function getSecurity()
	{
		...
		return view('spnet.config.security',$this->data);
	...
	public function postLogin( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			fwrite($fp,$val); 
			fclose($fp); 
			return Redirect::to('spnet/config/security')->with('messagetext','Setting Has Been Save Successful')->with('msgstatus','success');
		} else {
			return Redirect::to('spnet/config/security')->with('messagetext', 'The following errors occurred')->with('msgstatus','error')
			->withErrors($validator)->withInput();
		}	
	}
	...
	public function getLog( $type = null)
	{
		...
		return view('spnet.config.log',$this->data);
	}
	...
	public function getClearlog()
	{
		...
		return Redirect::to('spnet/config/log')->with('messagetext','Cache has been cleared !')->with('msgstatus','success');
	}
	...
	public function getTranslation( Request $request, $type = null)
	{
		...
		return view('spnet.config.translation.'.$template,$this->data);
	}
	
	public function getAddtranslation()
	{
		return view("spnet.config.translation.create");
	} 
	...
	public function postAddtranslation( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			return Redirect::to('spnet/config/translation')->with('messagetect','New Translation has been added !')->with('msgstatus','success');	;
			
		} else {
			return Redirect::to('spnet/config/translation')->with('messagetext','Failed to add translation !' )->with('msgstatus','error')->withErrors($validator)->withInput();
		}		
	...
	//	$filename = 'lang.php';
		...
		fwrite($fp,$form); 
		fclose($fp); 	
		return Redirect::to('spnet/config/translation?edit='.$lang.'&file='.$file)
	...
	public function getRemovetranslation( $folder )
	{
		...
		return Redirect::to('spnet/config/translation')->with('messagetext','Translation has been removed !')->with('msgstatus','success');
