/* Spnet builder
 copyright 2014 . spnet builder com
 */
...
jQuery(document).ready(function ($) {
	...
	if ($.cookie("spnet-sidebar") == 'minimize-sidemenu') {
	...
	$('.navbar-minimalize').click(function () {
		...
		if (w.hasClass('minimize-sidemenu')) {
			$('#sidemenu').removeClass('expanded-menu');
			$.cookie("spnet-sidebar", 'minimize-sidemenu', {expires: 365, path: '/'});
		} else {
			$('#sidemenu').addClass('expanded-menu');
			$.cookie("spnet-sidebar", 'maximaze-sidemenu', {expires: 365, path: '/'});
		}
	});
	;
	;
...
function SpnetConfirmDelete(url) {
	if (confirm('Are u sure deleting this record ? ')) {
		window.location.href = url;
	}
	return false;
}
function SpnetDelete() {
	var total = $('input[class="ids"]:checkbox:checked').length;
	if (confirm('are u sure removing selected rows ?')) {
		$('#SpnetTable').submit();// do the rest here
	}
}
function SpnetModal(url, title) {
	$('#spnet-modal-content').html(' ....Loading content , please wait ...');
	$('.modal-title').html(title);
	$('#spnet-modal-content').load(url, function () {
	});
	$('#spnet-modal').modal('show');
}
