...
class NotificationController extends Controller
{
	...
	function postSave(Request $request)
	{
		...
		else
		{
			...
			return Redirect::to('notification/update/' . $id)->with('messagetext', \Lang::get('core.note_error'))->with('msgstatus', 'error')->withErrors($validator)->withInput();
		}
	...
	public function postDelete(Request $request)
	{
		...
		if (count($request->input('id')) >= 1)
		{
			...
			// redirect
			return Redirect::to('notification')->with('messagetext', \Lang::get('core.note_success_delete'))->with('msgstatus', 'success');

		}
		else
		{
			return Redirect::to('notification')->with('messagetext', 'No Item Deleted')->with('msgstatus', 'error');
		}
	...
	public function getLoad()
	{
		...
		foreach ($result as $row)
		{
			if (++$i <= 10)
			{
				$data[] = array('url' => $row->url, 'title' => $row->title, 'icon' => $row->icon, 'date' => date("d/m/y", strtotime($row->created)));
			}
		}
